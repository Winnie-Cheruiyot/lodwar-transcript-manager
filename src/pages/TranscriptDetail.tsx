...
const TranscriptDetail = () => {
  ...
  useEffect(() => {
    ...
    if (!foundTranscript) { toast.error("Transcript not found"); navigate("/students"); return; }
  ...
  if (!transcript) {
    return <div className="container mx-auto py-8 px-4 text-center"><div className="animate-pulse p-8">Loading transcript...</div></div>;
  }

  return (
    <div className="container mx-auto py-8 px-4">
      <div className="flex justify-between items-center mb-8 print:hidden">
        <div><Button variant="outline" onClick={() => navigate("/students")}>← Back to Students</Button></div>
        <div className="flex gap-2">
          {isEditing ? (
            <div className="text-sm text-gray-600 font-medium mt-2">Editing transcript...</div>
          ) : (
            <>
              <Button variant="outline" onClick={() => setIsEditing(true)}>Edit Transcript</Button>
      ...
      <div className="print-container">
        {isEditing ? (
          <TranscriptEditor transcript={transcript} onSave={() => setIsEditing(false)} onCancel={() => setIsEditing(false)} />
        ) : (
          <TranscriptView transcript={transcript} isPrinting={isPrinting} />
        )}
      </div>
      
      <div className="mt-8 text-center text-sm text-gray-500 print:hidden">© Examination Department @ 2026 LVTC. All Rights Reserved.</div>
    </div>
  );
};
